...
开发服务只负责前端页面热更新；聊天、历史消息和 OSS 签名接口默认请求 `http://192.168.2.2:8001`。
...
原因通常是前端请求 `http://192.168.2.2:8001`，但 FastAPI 后端没有启动、电脑防火墙未放行 `8001`，或者手机和电脑不在同一局域网。
